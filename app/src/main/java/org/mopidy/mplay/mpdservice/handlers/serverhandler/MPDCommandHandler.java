...
public class MPDCommandHandler extends MPDGenericHandler {
    ...
    public static void toggleLike(String trackUri, boolean like) {
        MPDHandlerAction action = new MPDHandlerAction(MPDHandlerAction.NET_HANDLER_ACTION.ACTION_LIKE);
        Message msg = Message.obtain();
        if (msg == null) {
            return;
        }
        if(like)
          action.setStringExtra(MPDHandlerAction.NET_HANDLER_EXTRA_STRING.EXTRA_PLAYLIST_NAME, "liked:true:"+trackUri); else
            action.setStringExtra(MPDHandlerAction.NET_HANDLER_EXTRA_STRING.EXTRA_PLAYLIST_NAME, "liked:false:"+trackUri);
        ...
        try {
            ...
            if (action == MPDHandlerAction.NET_HANDLER_ACTION.ACTION_COMMAND_NEXT_SONG) {
                ...
                if (state == MPDCurrentStatus.MPD_PLAYBACK_STATE.MPD_PAUSING) {
                    WSInterface.getGenericInstance().pause(false);
                } else {
                    WSInterface.getGenericInstance().playSongIndex(status.getCurrentSongIndex());
                }
                ...
                WSInterface.getGenericInstance().setRandom(random);
                ...
                WSInterface.getGenericInstance().setRepeat(repeat);
                ...
                WSInterface.getGenericInstance().setSingle(single);
                ...
                WSInterface.getGenericInstance().setConsume(consume);
                ...
                WSInterface.getGenericInstance().playSongIndex(index);
                ...
                while (seekTo != mRequestedSeekVal) {
                    ...
                    WSInterface.getGenericInstance().seekSeconds(seekTo);
                }
                ...
                while (volume != mRequestVolume) {
                    ...
                    WSInterface.getGenericInstance().setVolume(volume);
                }
                ...
                WSInterface.getGenericInstance().toggleOutput(outputID);
            } else if (action == MPDHandlerAction.NET_HANDLER_ACTION.ACTION_ENABLE_OUTPUT) {
                int outputID = mpdAction.getIntExtra(MPDHandlerAction.NET_HANDLER_EXTRA_INT.EXTRA_OUTPUT_ID);
                WSInterface.getGenericInstance().enableOutput(outputID);
            } else if (action == MPDHandlerAction.NET_HANDLER_ACTION.ACTION_DISABLE_OUTPUT) {
                int outputID = mpdAction.getIntExtra(MPDHandlerAction.NET_HANDLER_EXTRA_INT.EXTRA_OUTPUT_ID);
                WSInterface.getGenericInstance().disableOutput(outputID);
                ...
                WSInterface.getGenericInstance().setVolume(targetVolume);
